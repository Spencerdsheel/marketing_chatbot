...
/**
 * Onboarding form + one-time result view (S13.2 decisions 2 & 3).
 *
 * Secrets hygiene (highest priority): `clientKey`/`generatedPassword` live
 * only in this component's React state (itself derived from the server
 * action's return value via `useActionState`) -- never written to a cookie,
 * `localStorage`, the URL, or `console`. Reloading this page loses them by
 * design; the "I have saved these" checkbox exists to stop an admin from
 * navigating away by accident before they've copied them.
 */
...
import { onboardTenant, type OnboardState } from "@/app/(protected)/tenants/new/actions";
...
/**
 * Copy-to-clipboard with a visible fallback when the Clipboard API is
 * unavailable (non-HTTPS/older browser) -- decision 3: "no silent no-op
 * button."
 */
...
function ResultView({ state }: { state: Extract<OnboardState, { status: "created" }> }) {
  ...
  const copyAllText = [
    `Tenant: ${state.tenant.name} (${state.tenant.slug})`,
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        <p className="text-sm">
          Tenant <span className="font-medium">{state.tenant.name}</span> (
...
export function OnboardForm() {
  const [state, formAction] = useActionState(onboardTenant, initialState);
