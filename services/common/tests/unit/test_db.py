...
async def test_connect_forwards_statement_cache_size_when_set() -> None:
    ...
    fake_pool = AsyncMock()
    with patch("common.db.asyncpg.create_pool", new_callable=AsyncMock, return_value=fake_pool) as mock:
        await Database.connect("postgres://fake", statement_cache_size=0)
...
async def test_connect_omits_statement_cache_size_when_none() -> None:
    ...
    fake_pool = AsyncMock()
    with patch("common.db.asyncpg.create_pool", new_callable=AsyncMock, return_value=fake_pool) as mock:
        await Database.connect("postgres://fake")
        mock.assert_called_once()
        _, kwargs = mock.call_args
